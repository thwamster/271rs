...
fn ecdsa(input : &str, seed : ix) {
	...
	let r : ix =
		hashing((hashing(message.to(10).as_str()) + message.clone()).to(10).as_str()) % p.clone();
	println!("r: {}", r);
	...
	let h : ix =
		hashing((R.clone().0 + public_key.clone().0 + message.clone()).to(10).as_str()) % p.clone();
	println!("h: {}", h);
	...
	let P2 : (ix, ix) = point_addition(
		R.clone(),
		apply_double_and_add_method(public_key.clone(), h.clone(), a.clone(), d.clone(), p.clone()),
		a.clone(),
		d.clone(),
		p.clone()
	);
...
fn apply_double_and_add_method(P : (ix, ix), k : ix, a : ix, d : ix, md : ix) -> (ix, ix) {
	...
	for (i, c) in k_as_binary.chars().enumerate() {
		...
		if c == '1' {
			addition_point =
				point_addition(addition_point.clone(), P.clone(), a.clone(), d.clone(), md.clone());
		}
	}
...
fn hashing(message : &str) -> ix { ix::from(&format!("0x{}", hash::hash(message).as_str())) }
