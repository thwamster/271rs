...
impl ix {
	pub fn new() -> ix { ix { sign : 0, bytes : vec![0] } }

	pub fn one() -> ix { ix { sign : 0, bytes : vec![1] } }

	pub fn from(input : &str) -> ix {
		let mut result : ix = ix { sign : 0, bytes : Vec::<u8>::new() };
		...
		if radix & (radix - 1) == 0 {
			...
			while nums.len() != 0 {
				if nums.last().unwrap().to_digit(radix as u32).expect("Invalid Input") % 2 != 0 {
					byte += 1 << i;
				}
	...
	pub fn from_bytes(bytes : Vec<u8>) -> ix { ix { sign : 0, bytes } }
...
impl Clone for ix {
	fn clone(&self) -> ix {
		let mut c : ix = ix { sign : self.sign, bytes : self.bytes.clone() };
...
impl ops::Div<ix> for ix {
	...
	fn div(self, x : ix) -> ix { ix::divmod(self, x).0 }
}

impl ops::Rem<ix> for ix {
	type Output = ix;

	fn rem(self, x : ix) -> ix { ix::divmod(self, x).1 }
}
...
impl cmp::PartialEq for ix {
	fn eq(&self, x : &ix) -> bool { self.sign == x.sign && self.bytes == x.bytes }
}
